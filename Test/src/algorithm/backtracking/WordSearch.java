...
public class WordSearch {
        ...
        for (int x = 0; x < sizex; x++) {
            for (int y = 0; y < sizey; y++) {
                if (board[y][x] == word.charAt(0))
                    backtrack(board, new Cordinate(x, y), 0, word);
                if (solution)
                    return true;
            }
        }
    ...
    private void backtrack(char[][] board, Cordinate c, int k, String word) {
        if (k == word.length()) {
            ...
            if (c.getX() < 0 || c.getX() >= board[0].length || c.getY() < 0
                    || c.getY() >= board.length) {
                return;
            }

            if (board[c.getY()][c.getX()] != word.charAt(k))
                return;
            if (mark[c.getY()][c.getX()])
                return;
    ...
    public static void main(String[] args) {
	    ...
	    System.out.println(wordSearch.exists(null, ""));
    }
}
